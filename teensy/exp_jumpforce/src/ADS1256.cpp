/*
        ...
        Modified Apr 2023 by Christopher Xu for custom pins
*/
...
ADS1256::ADS1256(uint8_t _pinCS, uint8_t _pinDRDY, uint8_t _pinSYNC, float _clockMhz, float vref) {
...
unsigned char ADS1256::readRegister(unsigned char reg) {
    ...
    delayNanoseconds(2000);                 //  t11 delay (4*tCLKIN 4*0.13 = 0.52 us)
    CSOFF();
    return readValue;
}
...
void ADS1256::sendCommand(unsigned char reg) {
    ...
    waitDRDY();
    SPI.transfer(reg);
    delayNanoseconds(2000); // intended t11 delay of 0.5us causes coupling between channels
    CSOFF();
}
...
void ADS1256::begin(unsigned char drate, unsigned char gain, bool buffenable) {
    // Start SPI on a quarter of ADC clock speed (maximum)
    SPI.begin();
    SPI.beginTransaction(SPISettings(clockMhz * 1000000 / 4, MSBFIRST, SPI_MODE1));
